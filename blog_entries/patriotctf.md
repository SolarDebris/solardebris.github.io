...
We cannot have any 0x3b, 0x2f, 0x62, 0x69, 0x6e, 0x73, 0x68, or 0x0. This means that we can't use
execve syscall (0x3b), we can't encode any pointers (0x0), and we can't write "flag" or "/".
...
>
> Another tip is to use any values in registers that are leftover when executing your shellcode, ie pointers,
> and other values that you can use later.
...
3. the read syscall will be in the binary. (How else can you read in shellcode in the first place, and any data).
4. the state of the registers will be more consistent, unless the registers are cleared before jumping to the shellcode.
...
In our current state, the only thing we have is that rdx is our shellcode buffer. So we can use these gadgets:
* xchg rdx, rsi gadget to set rsi
* xchg rdx, reg to set the value to a reasonable number
* xor rdi, rdi to set it to 0
